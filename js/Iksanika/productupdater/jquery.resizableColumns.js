...
(function($, window) {
  ...
  ResizableColumns = (function() {
    ...
    //      return this.$table.data('resizable-columns-id') + '-' + $el.data('resizable-column-id');
        ...
        /* START: Iksanika adjustment */
        if(($el.data('resizable-column-id')+'') == 'undefined')
        {
            return false;
        }else
        {
...
*/
    ResizableColumns.prototype.saveColumnWidths = function() {
        window.columnWidths = {};
//        window.columnWidths = '';
        var columnWidths2 = this.$tableHeaders.each((function(_this) {
            return function(_, el) {
              var $el;
              $el = $(el);
              if ($el.attr('data-noresize') == null) {
                  if(_this.getColumnId($el))
                  {
//                    console.debug('saveColumnWidths: '+_this.getColumnId($el)+' = '+parseWidth($el[0]));
                  }
                  window.columnWidths[_this.getColumnId($el)] = parseWidth($el[0]);
//                  window.columnWidths += '&fields['+_this.getColumnId($el)+']='+parseWidth($el[0]);
                  
//                  console.debug(window.columnWidths);
                    if(_this.options.store != null) 
                    {
//                        console.debug('_this.options.store: SET');
  //                      _this.options.store.set(_this.getColumnId($el), parseWidth($el[0]));
                    }
//                  console.debug('_this.options.store: '+_this.options.store);
//                if (_this.options.store != null) {
//                  return _this.options.store.set(_this.getColumnId($el), parseWidth($el[0]));
//                }
              }
...
//          var postData = 'form_key='+this.options.formKey+window.columnWidths;
            window.columnWidths['form_key'] = this.options.formKey;
          var postData = 'form_key='+this.options.formKey+'&fields='+JSON.stringify(window.columnWidths);
//          console.debug('columnWidths: '+JSON.stringify(window.columnWidths));
        // save columns order
        console.debug('saveWidthUrl: '+ this.options.resizedWidthUrl);
        new Ajax.Request(this.options.resizedWidthUrl, {
            method: 'post',
            postBody : postData,
...
//          if ((_this.options.store != null) && (width = _this.options.store.get(_this.getColumnId($el)))) {
          if ((window.columnWidths != null) && (width = window.columnWidths[_this.getColumnId($el)])) {
            return setWidth($el[0], width);
          }
        };
      })(this));
