...
varienGridMassaction.prototype = {
    ...
    initialize: function (containerId, grid, checkedValues, formFieldNameInternal, formFieldName) {				
        ...
        this.checkedSortOrderString = checkedValues;       
    ...
    apply: function() {
        ...
        if(item.id=='sort_order')
			new Insertion.Bottom(this.formHiddens, this.fieldTemplate.evaluate({name: fieldName, value: this.initMassSelectSorting()}));
		else
    ...
    initMassSelectSorting: function() {		
		haystack = '';
        $$('input[class~="massaction-input"]').each(function(element) {				
				str = element.id+':'+element.value;				
				haystack += str + ',';								                
            }.bind(this)
        );
        return haystack;
