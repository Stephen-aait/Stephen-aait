...
{
	$rpl = array( '\\' => '\\\\', '"' => '\\"' ) ;
	$output = '<html><body><script type="text/javascript">if (!(window.parent.CKEDITOR == null)){window.parent.CKEDITOR.tools.callFunction(1,"'.strtr( $fileUrl, $rpl ).'","'.strtr( $customMsg, $rpl ).'");}</script>';
	$output .= '<script type="text/javascript">' ;
	$output .= 'if (!(window.parent.OnUploadCompleted == null)){window.parent.OnUploadCompleted(' . $errorNumber . ',"' . strtr( $fileUrl, $rpl ) . '","' . strtr( $fileName, $rpl ) . '", "' . strtr( $customMsg, $rpl ) . '") ;}'. '</script>' ;
	$output.='</body></html>';
	echo $output;
	exit;
