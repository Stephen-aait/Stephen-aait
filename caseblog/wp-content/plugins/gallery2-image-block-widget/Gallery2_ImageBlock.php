...
// 0.1.5  2009-08-13 Add locally defined sanitize_key_names function, Valid for WP2.8.4

  function sanitize_key_names($name) {
  ...
  function widget_G2_ImageBlock_control() {
    ...
    if ( $_POST['G2_ImageBlock-submit'] ) {
      ...
      foreach($options as $key => $value)
        if(array_key_exists('G2_ImageBlock-'.sanitize_key_names($key), $_POST))
          $options[$key] = strip_tags(stripslashes($_POST['G2_ImageBlock-'.sanitize_key_names($key)]));
    ...
    foreach($options as $key => $value): ?>
      <p style="text-align:left"><label for="G2_ImageBlock-<?=sanitize_key_names($key)?>"><?=$key?>: <input style="width: 200px;" id="G2_ImageBlock-<?=sanitize_key_names($key)?>" name="G2_ImageBlock-<?=sanitize_key_names($key)?>" type="text" value="<?=htmlspecialchars($value, ENT_QUOTES)?>" /></label></p>
    <? endforeach;
