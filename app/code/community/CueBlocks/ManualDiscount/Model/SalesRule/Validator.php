 ...
 */
class CueBlocks_ManualDiscount_Model_SalesRule_Validator extends Mage_SalesRule_Model_Validator
{


    /*
     * Function overrided to prevent force setting item discount to zero when applying Manual discount in admin
     * */
    /**
    ...
    {
        $postCustomDiscount = Mage::app()->getRequest()->getPost('customdiscount');
        //If we are applying custom discount then don't reset item totals
		if (!isset($postCustomDiscount) || empty($postCustomDiscount)) {
        ...
        foreach ($this->_getRules() as $rule) {
            ...
            Mage::dispatchEvent(
                'salesrule_validator_process', array(
                'rule'    => $rule,
                'item'    => $item,
                'address' => $address,
                'quote'   => $quote,
    ...
    /*
     * Function overrided to fix removal of discount description in case of manual discount
     * */
    /**
    ...
    public function prepareDescription($address, $separator=', ')
    {
        ...
        if (is_array($description) && !empty($description)) {
            ...
            if ($address->getDiscountAmount() !=0) {
                $description = Mage::getModel('sales/quote_address')->load($address->getAddressId())->getDiscountDescription();
            } else {
                $description = '';
            }
        }
