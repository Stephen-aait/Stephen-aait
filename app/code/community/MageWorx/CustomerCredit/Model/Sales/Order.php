...
class MageWorx_CustomerCredit_Model_Sales_Order extends Mage_Sales_Model_Order {
    ...
    protected function _checkState() {
        ...
        if (!$this->getCustomerCreditAmount()) {
        ...
        if (!$this->isCanceled()
            ...
            if($this->getState() !== self::STATE_CLOSED && abs($this->getTotalOfflineRefunded()-($this->getGrandTotal()+$this->getCustomerCreditAmount())) < 0.0001) {
                $this->_setState(self::STATE_CLOSED, true, '', $userNotification);
                $this->unsForcedCanCreditmemo();
            }
        }
