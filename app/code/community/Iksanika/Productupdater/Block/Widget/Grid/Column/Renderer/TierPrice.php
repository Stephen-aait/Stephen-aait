...
//        $this->f->$method($args[0]);
    ...
    public function render(Varien_Object $row, $isAssociated = false) 
    {
        ...
        $_htmlName  =   'tier_price' . ($isAssociated ? '_assoc' : '');
        
//$_showWebsite = $this->isShowWebsiteColumn();
//$_editWebsite = $this->isAllowChangeWebsite();
        ...
        $html .= '
                ...
                <tfoot>
                    <tr>
                        ...
                        <td colspan="4" class="a-right"><button id="id_add_'.$_htmlName.'" title="'.Mage::helper('catalog')->__('Add Tier').'" type="button" class="scalable add" onclick="return tierPriceControl'. ($isAssociated ? 'Assoc' : '').$_htmlId.'.addItem()" style=""><span>'.Mage::helper('catalog')->__('Add Tier Price').'</span></button></td>
                    </tr>
                </tfoot>
            </table>
        ';
...
//<![CDATA[
var tierPriceRowTemplate'. ($isAssociated ? 'Assoc' : '').$_htmlId.' = \'<tr>\'
    ...
    $html .= '+ \'</select></td>\'
    + \'<td class="nobr"><input class="'.$_htmlClass.' qty required-entry validate-greater-than-zero" type="text" name="'.$_htmlName.'[{{index}}][price_qty]" value="{{qty}}" id="'.$_htmlName.'_row_{{index}}_qty" />\'
    ...
    + \'<button title="'.Mage::helper('catalog')->__("Delete Tier").'" type="button" class="scalable delete icon-btn delete-product-option" id="'.$_htmlName.'_row_{{index}}_delete_button" onclick="return tierPriceControl'.$_htmlId.'.deleteItem(event);">\'
    + \'<span><span><span>'.Mage::helper('catalog')->__("Delete").'</span></span></span></button></td>\'
    + \'</tr>\';

var tierPriceControl'. ($isAssociated ? 'Assoc' : '').$_htmlId.' = {
    template: new Template(tierPriceRowTemplate'. ($isAssociated ? 'Assoc' : '').$_htmlId.', new RegExp(\'(^|.|\\\\r|\\\\n)({{\\\\s*(\\\\w+)\\\\s*}})\', "")),
    ...
    addItem : function () {
        ...
        var wss = $j(\'#product'.$_htmlId.' #'.$_htmlName.'_row_\'+data.index+\'_website\');
    ...
    deleteItem: function(event) {
        ...
        {
            $html .= '
                    tierPriceControl'. ($isAssociated ? 'Assoc' : '').$_htmlId.'.addItem(\''.$_item['website_id'].'\', \''.$_item['cust_group'].'\', \''.($_item['price_qty']*1).'\', \''.sprintf('%.2f', $_item['price']).'\', '.(int)!empty($_item['readonly']).');';
        }
        if ($_readonly)
        {
            $html .= '$(\''.$_htmlName.'_'.$_htmlId.'_container\').up(\'table\').select(\'button\')
                .each(tierPriceControl'. ($isAssociated ? 'Assoc' : '').$_htmlId.'.disableElement);';
        }
