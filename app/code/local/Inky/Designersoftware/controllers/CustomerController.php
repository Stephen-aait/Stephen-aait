...
class Inky_Designersoftware_CustomerController extends Mage_Core_Controller_Front_Action
{
   /**
    ...
    public function designAction(){
		$removeItemId = $this->getRequest()->getParam('remove');
		...
		if( !empty($removeItemId) && $removeItemId>0 ){
			$dataDesignersoftwareArray['update_designersoftware_ids'] = serialize(array($removeItemId));
			$dataDesignersoftwareArray['remove_status'] = 1;
						
			Mage::helper('designersoftware/designersoftware_updateattributes')->updateAttributes($dataDesignersoftwareArray);
			Mage::getSingleton('core/session')->addSuccess('Design removed Successfully.');
			
			$this->_redirect('designersoftware/customer/design');
            return;
		}
	...
	public function ajaxcommentAction(){
        ...
        $description 		= $this->getRequest()->getPost('desc');
        $designersoftwareId = $this->getRequest()->getPost('id');
        
        if(!empty($description) && $designersoftwareId>0){
			$designCollection = Mage::getModel('designersoftware/designersoftware')->load($designersoftwareId);
			$designCollection->setContent($description);
