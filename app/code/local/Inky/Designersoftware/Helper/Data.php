...
{	
	...
	public function getUsedColor(){
		...
		if($collection->getSize()>0){
			foreach($collection->getData() as $value){
				$colorArr[$value['color_id']]=$value['title'];								
			}
		}
		return $colorArr;
	}
	
	public function getModelCode($modelId){
		$collection = Mage::getModel('designersoftware/model')->getCollection()
	...
	public function getUsedColorsString($chinese_info_arr){
		$chineseInfoArr = unserialize($chinese_info_arr);
		$colorArr =  array();
		$colorString = '';
		foreach($chineseInfoArr as $value){
			if(!in_array($value['colorId'], $colorArr)){
				$colorArr[] = $value['colorId'];
				$collection = Mage::getModel('designersoftware/color')->getCollection()
									->addFieldToFilter('color_id', $value['colorId'])
									->getFirstItem();
					
				$colorString .= $collection->getTitle() . ',';
			}
		}
