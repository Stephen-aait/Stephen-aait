...
class Webtex_Giftcards_Block_Adminhtml_Catalog_Product_Tab_Pregenerated extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareLayout()
    {
        ...
        $this->getUploader()->getConfig()
            ->setUrl(Mage::getModel('adminhtml/url')->addSessionParam()->getUrl('adminhtml/product/import/id/'.$this->_getProduct()->getId()))
            ->setFileField('file')
            ->setFilters(array(
                'csv' => array(
        ...
        $this->addColumn('card_delete', array(
            ...
            'actions'    => array(
                array(
                    ...
                    'url'       => array('base' => 'adminhtml/product/deletecard', 'params' => array('_current' => true)),
                    'field'     => 'card_id',
                ),
            ),
        ));
             ...
             $addhtml = '<div class="entry-edit">
                    <div class="entry-edit-head"><h4 class="icon-head head-edit-form fieldset-legend">Generate Gift Card Codes</h4>
                        <div class="form-buttons">
                            <button id="create_giftcards" title="Generate" type="button" class="scalable add" onclick="generateGiftCards(\''.$this->getUrl('adminhtml/product/generate').'\','.$this->_getProduct()->getId().');"><span><span><span>Generate</span></span></span></button>
                        </div>
                    </div>
                     <div class="grid">
