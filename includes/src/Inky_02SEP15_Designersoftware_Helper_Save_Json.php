...
{		
	...
	public function load($data){
		Mage::log($data['chineseInfoArr'],null,'designersoftware.log');										
		...
		try 
		{
			...
			if($data['eventType']==self::DESIGNERSOFTWRE_ADD_TO_CART):
				...
				$returnData['sessionId']	= Mage::helper('designersoftware/customer')->getCurrentSessionId();
				$returnData['styleDesignCode'] = $data['styleDesignCode'];
	...
	public function setDesignerData($data){
		...
		// General Data
		$designerData['designId']				= $data['designId'];
		...
		// Price Data					
		//$designerData['base_price']				= Mage::getModel('designersoftware/style')->convertCurrentToBaseCurrency($data['basePrice']);
		//$designerData['fashion_lining']			= Mage::getModel('designersoftware/style')->convertCurrentToBaseCurrency($data['fashion_lining']);
