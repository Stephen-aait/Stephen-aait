...
{    
    ...
    public function generateAction(){
		...
		// Designed Images 
		$images 			= $this->designImages($styleDesignCode);		
	...
	public function designImages($styleDesignCode){
	...
	public function getTranslation($priceInfoArrValue, $storeId, $partIden)
	{
		...
		else if($partIden=="clipartId")
		{
			if(isset($priceInfoArrValue->clipartId) && !empty($priceInfoArrValue->clipartId) && $priceInfoArrValue->clipartId > 0)
			{
				...
				$value = Mage::getModel('designersoftware/clipart')->getCollection()->addFieldToSelect('title')->addFieldToFilter('clipart_id',$clipartId)->getFirstItem()->getData();			
				return $value['title'];
			}
		}
