...
class Mage_Catalog_Block_Product_List_Toolbar extends Mage_Core_Block_Template
{
    ...
    public function setCollection($collection)
    {
        ...
        if ($this->getCurrentOrder()) {
			if($this->getCurrentOrder() == 'qty') {
                $this->getCollection()->getSelect()
                     ->joinLeft(
                            array('sfoi' => $collection->getResource()->getTable('cataloginventory/stock_item')),
                             'e.entity_id = sfoi.product_id'
                             ,array('qty' => 'sfoi.qty')
                         )
                     ->group('e.entity_id')
                     ->order('sfoi.qty DESC');
                     //->order('sfoi.qty ' . $this->getCurrentDirection());
            } elseif($this->getCurrentOrder() == 'nis') {
                $this->getCollection()->getSelect()
                     ->joinLeft(
                            array('sfoi' => $collection->getResource()->getTable('cataloginventory/stock_item')),
                             'e.entity_id = sfoi.product_id'
